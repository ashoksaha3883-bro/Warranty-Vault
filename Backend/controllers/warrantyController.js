...
const Warranty = require("../modal/Warrenty.js");
...
const scanWarranty = async (req, res) => {
  try {
    // -----------------------------------------------
    // CHECK FILE
    // -----------------------------------------------
    ...
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: "No warranty document was uploaded.",
      });
    }
    ...
    //
    // IMAGE:
    // Compress only the copy that goes to OCR.
    //
    // PDF:
    // Send original PDF without compression.
    //
    // IMPORTANT:
    // req.file.buffer itself is NOT changed.
    // Your original uploaded document remains available
    // for the frontend/local document storage.
    // -----------------------------------------------

    let ocrBuffer = req.file.buffer;
    let ocrMimeType = req.file.mimetype;
    let ocrFileName =
      req.file.originalname || "warranty-document";
    ...
    if (
      ...
      ocrBuffer = compressed.buffer;
      ocrMimeType = compressed.mimeType;
      ocrFileName = compressed.fileName;

      console.log(
        `Warranty image compressed: ${Math.round(
          req.file.size / 1024
        )} KB -> ${Math.round(
          ocrBuffer.length / 1024
        )} KB`
      );
    }
    ...
    form.append("file", ocrBuffer, {
      filename: ocrFileName,
      contentType:
        ocrMimeType || "application/octet-stream",
    });
    ...
    const extracted = extractWarrantyData(rawText);
    ...
    console.log(
      `OCR completed: ${
        extracted.productName || "Unknown product"
      }`
    );
...
const compressImageForOCR = async (inputBuffer) => {
  ...
  for (const quality of qualities) {
    ...
    if (output.length <= maxBytes) {
      return {
        buffer: output,
        mimeType: "image/jpeg",
        fileName: "warranty-document.jpg",
      };
    }
  }
...
const createWarranty = async (req, res) => {
  try {
    ...
    if (
      req.user.emailNotificationsEnabled === true
    ) {
      sendWarrantySavedConfirmationEmail({
        user: req.user,
        warranty,
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to save warranty",
    });
  }
};
...
const getMyWarranties = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to get warranties",
    });
  }
};
...
function extractWarrantyData(text) {
  ...
  const purchaseDate =
    extractPurchaseDate(cleanedText);

  const invoiceNumber =
    extractInvoiceNumber(cleanedText);

  const seller =
    extractSeller(cleanedText);

  const contact =
    extractSellerContact(cleanedText);

  const placeOfSupply =
    extractPlaceOfSupply(cleanedText);
...
function extractWarrantyStartDate(text) {
  const patterns = [
...
function extractWarrantyEndDate(text) {
  const patterns = [
...
function extractWarrantyDuration(text) {
  const patterns = [
...
function isUsefulInvoiceNumber(value) {
  if (!value) {
    return false;
  }
...
function extractSellerContact(text) {
  const sellerIndex =
    text.search(
      /CITY COMPUTER/i
    );
...
function extractWarrantyType(text) {
  if (
    /manufacturer\s+warranty/i.test(text)
  ) {
    return "Manufacturer Warranty";
  }

  if (
    /third[\s-]?party\s+warranty/i.test(text)
  ) {
    return "Third-Party Warranty";
  }

  if (
    /extended\s+warranty/i.test(text)
  ) {
    return "Extended Warranty";
  }

  if (
    /warranty/i.test(text)
  ) {
    return "Warranty";
  }
...
function extractProducts(text) {
  ...
  for (
    ...
    if (
      !isUsefulProductName(name)
    ) {
      continue;
    }

    const brand =
      detectProductBrand(name);
...
function extractProductsFallback(lines) {
  const products = [];
...
function detectProductBrand(name) {
  const brands = [
    "Intel",
    "Adata",
    "Crucial",
